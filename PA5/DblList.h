...
// CPSC350
// PA5
// doubly linked list for advisees list
...
template<typename T>
int DblList<T>::removeData(T data){
    //make sure not empty
    int pos = 0;
...
template<typename T>
void DblList<T>::print(std::ostream &os) {
    ListNode<T>* current = m_front;
    while (current != m_back) {
        os << current->m_data << ", ";
        current = current->m_next;
    }
    os << m_back->m_data << std::endl;
    return;
}
